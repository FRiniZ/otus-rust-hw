...
use udp_msg_lib::proto::{thermo_msg_new, thermo_msg_pack};
...
impl ThermoUDPGen {
    ...
    fn serv(&self) {
        ...
        let mut counter = 1;
        loop {
            let msg = thermo_msg_new(Some(20.0 + ((counter as f32) / 2.0).sin()));
            counter += 1;
            println!("The temperature is {}", msg.temperature.unwrap());

            let body = thermo_msg_pack(&msg);
