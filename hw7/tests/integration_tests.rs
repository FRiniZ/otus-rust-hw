...
use hw7::{
    ...
    smarthouse::{BorrowingDeviceInfoProvider, OwningDeviceInfoProvider, SmartHouse},
};
...
fn smartroom() {
    ...
    let mut socket1 = SmartDevice::from(SmartSocket::new("ssocket1", "brief1"));
    let mut socket2 = SmartDevice::from(SmartSocket::new("ssocket1", "brief1"));
    assert!(room.add_device(&mut socket1).is_ok());
    assert!(room.add_device(&mut socket2).is_err());
...
fn smarthouse() {
    ...
    let mut socket1 = SmartDevice::from(SmartSocket::new("socket1", "brief1"));
    let mut socket2 = SmartDevice::from(SmartSocket::new("socket2", "brief2"));

    assert!(room1.add_device(&mut socket1).is_ok());
    assert!(room2.add_device(&mut socket2).is_ok());
...
fn negative_tests() {
    ...
    let mut socket1 = SmartDevice::from(SmartSocket::new("socket1", "brief1"));
    let mut socket2 = SmartDevice::from(SmartSocket::new("socket1", "brief2"));

    let ret = room1.add_device(&mut socket1);
    assert!(ret.is_ok());
    let ret = room1.add_device(&mut socket2);
    assert!(ret.is_err());
}
